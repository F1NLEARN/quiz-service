...
import com.finlearn.quizservice.quizetl.application.service.QuizIdPoolService;
...
public class CreateQuizSessionService {
    ...
    private final QuizIdPoolService quizIdPoolService;
    ...
    public CreateSessionResponse createLearningSession(CreateLearningSessionCommand command) {
        ...
        // Redis ID 풀에서 랜덤 문제 ID 추출 (DB 폴백 포함)
        List<UUID> randomQuizIds = quizIdPoolService.getRandomIds(mainTopic, DEFAULT_QUIZ_COUNT);
    ...
    public CreateSessionResponse createPointSession(CreatePointSessionCommand command) {
        ...
        // Redis ID 풀에서 랜덤 문제 ID 추출 (DB 폴백 포함)
        List<UUID> randomQuizIds = quizIdPoolService.getRandomIds(DEFAULT_QUIZ_COUNT);
